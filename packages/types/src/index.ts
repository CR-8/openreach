...
// WhatsApp Session & Messaging Provider
...
export interface ServerToClientEvents {
  ...
  'session:status': (data: { sessionId: string; status: SessionStatus; qrCodeBase64?: string }) => void;
  'workflow:node_start': (data: { runId: string; nodeId: string }) => void;
  'workflow:node_complete': (data: { runId: string; nodeId: string; output?: any }) => void;
  'workflow:node_error': (data: { runId: string; nodeId: string; error: string }) => void;
}
...
// Workflow Engine Node Categories & Types
